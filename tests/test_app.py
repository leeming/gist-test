...
def test_public_gist_no_history():
    ...
    assert len(public_gists.get_user_public_gist("ben")) == 11

@pytest.mark.parametrize('date, expected', [("2020-12-12T00:00:00Z",0),("2018-01-01T00:00:00Z",1),("",11)])
def test_public_gist_with_history(date, expected):
    public_gists.user_timestamps={'ben':date}
    assert len(public_gists.get_user_public_gist("ben")) == expected
...
def test_load_time_good_pickle(tmpdir):
    #Store empty dict
    public_gists.user_timestamps={'foobar':'2020-12-12T00:00:00.00000Z'}    
    ...
    assert public_gists.user_timestamps == {'foobar':'2020-12-12T00:00:00.00000Z'}
...
def test_e2e_user_gist_history_stored(tmpdir):
    ...
    assert public_gists.run("ben") == 11
    assert public_gists.run("ben") == 0